...
def fill_form(keys_with_lines):
    ...
    consumer_name = random_name()
    new_name = random_name()
    service_conn_no = f"SCN{random.randint(100000,999999)}"
    category = random.choice(CATEGORIES)
    contracted_load = round(random.uniform(1.0, 10.0), 2)
    ...
    contact_no = phone[-10:]
    email = f"{consumer_name.split()[0].lower()}@gmail.com"
    form_data = {}
    for idx, (key, line_no) in enumerate(keys_with_lines):
        ...
        value = ""
        if k == "serviceconnectionno":
            value = service_conn_no
        elif k == "nameofregisteredconsumeratpresent":
            value = consumer_name
        elif k == "consumercategory":
            value = category
        elif k == "contractedload":
            value = contracted_load
        elif k == "addresswithtelephonenumber":
            value = f"{address}, Tel: {phone}"
        elif k == "nameofthepersoninwhosenameconnectiontobechangedincapitalletter":
            value = new_name
        elif k == "date":
            value = today
...
def main():
    FORM_PATH = os.path.join(FORMS_DIR, 'ID_20.txt')
    OUTPUT_PATH = os.path.join(FORMS_FILL_DIR, 'ID_20_filled.txt')
